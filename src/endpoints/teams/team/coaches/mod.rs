...
pub struct CoachesEndpointUrl {
    team_id: TeamId,
    season: Option<u16>,
    date: Option<NaiveDate>,
}

impl Display for CoachesEndpointUrl {
...
mod tests {
    ...
    use crate::endpoints::teams::team::coaches::CoachesEndpointUrl;
    ...
    async fn test_heavy() {
        ...
        for team in teams.teams {
            let json = reqwest::get(CoachesEndpointUrl { team_id: team.id, season: Some(season), date: None }.to_string()).await.unwrap().bytes().await.unwrap();
            let mut de = serde_json::Deserializer::from_slice(&json);
            let result: Result<<CoachesEndpointUrl as StatsAPIUrl>::Response, serde_path_to_error::Error<_>> = serde_path_to_error::deserialize(&mut de);
            match result {
                Ok(_) => {}
