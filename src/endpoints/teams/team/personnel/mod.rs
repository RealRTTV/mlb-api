...
impl<S: personnel_endpoint_builder::State> crate::endpoints::links::StatsAPIEndpointUrlBuilderExt for PersonnelEndpointBuilder<S> where S: personnel_endpoint_builder::IsComplete {
    type Built = PersonnelEndpoint;
}
...
mod tests {
    ...
    async fn test_heavy() {
        ...
        for team in teams.teams {
            let _ = crate::serde_path_to_error_parse(PersonnelEndpoint::builder().team_id(team.id).season(season).build()).await;
        }
    }
}
